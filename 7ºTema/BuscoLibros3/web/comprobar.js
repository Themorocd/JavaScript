...
function iniciar() {
    ...
    texto.insertAdjacentHTML("afterend", sugerenciaDIV.outerHTML);
...
function pintodatos() {
    ...
    peticion.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
    let query = creaqueryinfopedidos();
    peticion.send(creaqueryinfopedidos());
}

function creaqueryinfopedidos() {
    texto = document.getElementById("texto").value;
    return "texto=" + encodeURIComponent(texto) + "&nocache=" + Math.random();
}
